      ...
      {postData.map((postItem) => (
        <div key={postItem.id} className="flex flex-col flex-wrap mb-6">
          <div className="w-full h-[13rem]">
            ...
            <img src={postItem.image} alt={`Cover for ${postItem.title}`} className="w-full h-full object-cover" />
            </div>
          </div>
          <div className="w-full">
            <div className="bg-gray-800 p-4 rounded-bl-lg rounded-br-lg h-64">
              <h1 className="text-lg mb-2 font-bold h-16 py-2 overflow-hidden">{postItem.title}</h1>
              <p className="text-sm text-gray-400 h-16 overflow-hidden">{postItem.content}</p>
